    ...
    public partial class exyt : Form
    {
        ...
        void LoadsAdds()
        {
            ...
            for (int l = 0; l < adds.Length; l++)
            {
                ...
                totalP = MainWindow_OLD.eLC.Lists[adds[l]].elementValues.Length;
                moveP = 0;
                for (int e = 0; e < MainWindow_OLD.eLC.Lists[adds[l]].elementValues.Length; e++)
                {
                    moveP++;
                    FILEs = MainWindow_OLD.eLC.Lists[adds[l]].elementValues[e].Length;
                    ark = 0;
                    for (int f = 46; f < MainWindow_OLD.eLC.Lists[adds[l]].elementValues[e].Length; f++)
                    {
                        var a = MainWindow_OLD.eLC.Lists[adds[l]].elementFields[f]; //var b = eLC.Lists[adds[l]].elementTypes[f]; 
                        ark++;

                        if (a.EndsWith("_id_addon") | a.EndsWith("_id_rand") | a.EndsWith("_id_unique"))
                        {
                            int c = int.Parse(MainWindow_OLD.eLC.GetValue(adds[l], e, f));
            ...
            if (File.Exists(Path.GetDirectoryName(Application.ExecutablePath) + @"\" + @"resources\opt\add_wepom.txt"))
            {
                File.Delete(Path.GetDirectoryName(Application.ExecutablePath) + @"\" + @"resources\opt\add_wepom.txt");
            }
            ...
            salvar1.Close();
            if (File.Exists(Path.GetDirectoryName(Application.ExecutablePath) + @"\" + @"resources\opt\add_armor.txt"))
            {
                File.Delete(Path.GetDirectoryName(Application.ExecutablePath) + @"\" + @"resources\opt\add_armor.txt");
            }
            ...
            salvar2.Close();
            if (File.Exists(Path.GetDirectoryName(Application.ExecutablePath) + @"\" + @"resources\opt\add_decoration.txt"))
            {
                File.Delete(Path.GetDirectoryName(Application.ExecutablePath) + @"\" + @"resources\opt\add_decoration.txt");
            }
